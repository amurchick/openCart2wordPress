	...
	var CP = class CP {
		...
		constructor(opts) {
			...
			cp.stderr.on('data', (data) => {
				...
				if (that.separatorRe && !msg.match(that.separatorRe))
					trace(`Child process ${title} stderr: ${data}`);
			...
			cp.stdout.on('data', (data) => {
				...
				trace(`Child process ${title} stdout: ${data}`);
		...
		send(cmd) {
			...
			trace(`Write to child process ${that.opts.cmd}: ${cmd}`);
